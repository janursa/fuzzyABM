...
data = {'S1':{'x':[],'y':[]},'S2':{'x':[],'y':[]},'S3':{'x':[],'y':[]},'S1-3':{'x':[],'y':[]}}
...
for (key,values),x in zip(calib_params.items(),xs):
	...
	if values[0] != None:
		...
		data['S1']['y'].append(standardized_value)
	if values[1] != None:
		data['S2']['x'].append(x)
		standardized_value = values[1]/np.mean(free_parameters[key])
		data['S2']['y'].append(standardized_value)
	if values[2] != None:
		data['S3']['x'].append(x)
		standardized_value = values[2]/np.mean(free_parameters[key])
		data['S3']['y'].append(standardized_value)
	if values[3] != None:
		data['S1-3']['x'].append(x)
		standardized_value = values[3]/np.mean(free_parameters[key])
		data['S1-3']['y'].append(standardized_value)

## settings
format = '.svg'
output_dir = os.path.join(current_file,'plots')
...
if __name__ == "__main__":
	...
	for key,values in data.items():
		ax.scatter(values['x'],values['y'] ,  
	               marker = symbols[i], s=150,color = colors[i],label = key)
		i+=1
	plt.xticks([(i) for i in range(len(xs))], labels)
